...
exports.OINOApi = exports.OINOApiHtmlTemplate = exports.OINOApiResult = exports.OINOApiRequest = void 0;
const node_buffer_1 = require("node:buffer");
const hashid_1 = require("@oino-ts/hashid");
const OINOQueryParams_js_1 = require("./OINOQueryParams.js");
const OINOConfig_js_1 = require("./OINOConfig.js");
const OINORequest_js_1 = require("./OINORequest.js");
const OINOResult_js_1 = require("./OINOResult.js");
const OINOHtmlTemplate_js_1 = require("./OINOHtmlTemplate.js");
const OINODataField_js_1 = require("./OINODataField.js");
const OINOBenchmark_js_1 = require("./OINOBenchmark.js");
class OINOApiRequest extends OINORequest_js_1.OINOHttpRequest {
    rowId;
    rowData;
    queryParams;
    constructor(init) {
        ...
        if (init?.filter) {
            if (init.filter instanceof OINOQueryParams_js_1.OINOQueryFilter) {
                this.queryParams.filter = init.filter;
            }
            else {
                this.queryParams.filter = OINOQueryParams_js_1.OINOQueryFilter.parse(init.filter);
            }
        }
        if (!this.queryParams.filter) {
            const filter_params = this.url?.searchParams.getAll(OINOConfig_js_1.OINOConfig.OINO_QUERY_FILTER_PARAM) || [];
            for (let i = 0; i < filter_params.length; i++) {
                const f = OINOQueryParams_js_1.OINOQueryFilter.parse(filter_params[i]);
                if (i > 0) {
                    this.queryParams.filter = OINOQueryParams_js_1.OINOQueryFilter.combine(this.queryParams.filter, OINOQueryParams_js_1.OINOQueryBooleanOperation.and, f);
                }
                else {
        ...
        if (init?.order) {
            if (init.order instanceof OINOQueryParams_js_1.OINOQueryOrder) {
                this.queryParams.order = init.order;
            }
            else {
                this.queryParams.order = OINOQueryParams_js_1.OINOQueryOrder.parse(init.order);
            }
        }
        if (!this.queryParams.order) {
            const order_param = this.url?.searchParams.get(OINOConfig_js_1.OINOConfig.OINO_QUERY_ORDER_PARAM);
            if (order_param) {
                this.queryParams.order = OINOQueryParams_js_1.OINOQueryOrder.parse(order_param);
            }
        }
        if (init?.limit) {
            if (init.limit instanceof OINOQueryParams_js_1.OINOQueryLimit) {
                this.queryParams.limit = init.limit;
            }
            else {
                this.queryParams.limit = OINOQueryParams_js_1.OINOQueryLimit.parse(init.limit);
            }
        }
        if (!this.queryParams.limit) {
            const limit_param = this.url?.searchParams.get(OINOConfig_js_1.OINOConfig.OINO_QUERY_LIMIT_PARAM);
            if (limit_param) {
                this.queryParams.limit = OINOQueryParams_js_1.OINOQueryLimit.parse(limit_param);
            }
        }
        if (init?.aggregate) {
            if (init.aggregate instanceof OINOQueryParams_js_1.OINOQueryAggregate) {
                this.queryParams.aggregate = init.aggregate;
            }
            else {
                this.queryParams.aggregate = OINOQueryParams_js_1.OINOQueryAggregate.parse(init.aggregate);
            }
        }
        if (!this.queryParams.aggregate) {
            const aggregate_param = this.url?.searchParams.get(OINOConfig_js_1.OINOConfig.OINO_QUERY_AGGREGATE_PARAM);
            if (aggregate_param) {
                this.queryParams.aggregate = OINOQueryParams_js_1.OINOQueryAggregate.parse(aggregate_param);
            }
        }
        if (init?.select) {
            if (init.select instanceof OINOQueryParams_js_1.OINOQuerySelect) {
                this.queryParams.select = init.select;
            }
            else {
                this.queryParams.select = OINOQueryParams_js_1.OINOQuerySelect.parse(init.select);
            }
        }
        if (!this.queryParams.select) {
            const select_param = this.url?.searchParams.get(OINOConfig_js_1.OINOConfig.OINO_QUERY_SELECT_PARAM);
            if (select_param) {
                this.queryParams.select = OINOQueryParams_js_1.OINOQuerySelect.parse(select_param);
            }
        }
    }
...
exports.OINOApiRequest = OINOApiRequest;
/**
 * OINO API request result object with returned data and/or http status code/message and
 * error / warning messages.
 *
 */
class OINOApiResult extends OINOResult_js_1.OINOResult {
...
exports.OINOApiResult = OINOApiResult;
/**
 * Specialized HTML template that can render ´OINOApiResult´.
 *
 */
class OINOApiHtmlTemplate extends OINOHtmlTemplate_js_1.OINOHtmlTemplate {
    ...
    async renderFromDbData(modelset, overrideValues) {
        OINOBenchmark_js_1.OINOBenchmark.startMetric("OINOHtmlTemplate", "renderFromDbData");
        let html = "";
        ...
        OINOBenchmark_js_1.OINOBenchmark.endMetric("OINOHtmlTemplate", "renderFromDbData");
        return result;
    }
}
exports.OINOApiHtmlTemplate = OINOApiHtmlTemplate;
/**
