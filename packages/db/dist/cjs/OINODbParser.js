...
class OINODbParser {
    ...
    static createRowsFromText(datamodel, data, requestParams) {
        if ((requestParams.requestType == index_js_1.OINOContentType.json) || (requestParams.requestType == undefined)) {
            ...
            index_js_1.OINOLog.error("@oinots/db", "OINODbParser", "createRowsFromText", "HTML can't be used as an input content type!", { contentType: index_js_1.OINOContentType.html });
            return [];
        }
        else {
            index_js_1.OINOLog.error("@oinots/db", "OINODbParser", "createRowsFromText", "Unrecognized input content type!", { contentType: requestParams.requestType });
            return [];
        }
    }
    /**
    ...
    static createRowsFromBlob(datamodel, data, requestParams) {
        if ((requestParams.requestType == index_js_1.OINOContentType.json) || (requestParams.requestType == undefined)) {
            ...
            index_js_1.OINOLog.error("@oinots/db", "OINODbParser", "createRowsFromBlob", "HTML can't be used as an input content type!", { contentType: index_js_1.OINOContentType.html });
            return [];
        }
        else {
            index_js_1.OINOLog.error("@oinots/db", "OINODbParser", "createRowsFromBlob", "Unrecognized input content type!", { contentType: requestParams.requestType });
            return [];
        }
    }
    /**
    ...
    static _createRowFromCsv(datamodel, data) {
        ...
        while (end < n) {
            ...
            if (has_data) {
                ...
                index_js_1.OINOLog.warning("@oinots/db", "OINODbParser", "_createRowFromCsv", "Empty row skipped", {});
            }
            start = end;
            end = start;
        }
    ...
    static _createRowFromJsonObj(obj, datamodel) {
        ...
        if (has_data) {
            ...
            index_js_1.OINOLog.warning("@oinots/db", "OINODbParser", "_createRowFromJsonObj", "Empty row skipped", {});
            return undefined;
        }
    }
    ...
    static _createRowFromFormdata(datamodel, data, multipartBoundary) {
        ...
        try {
            ...
            while (end < n) {
                ...
                if (!header_matches) {
                    index_js_1.OINOLog.warning("@oinots/db", "OINODbParser", "_createRowFromFormdata", "Unsupported block skipped", { header_line: l });
                    block_ok = false;
                }
                else {
                    ...
                    if (field_index < 0) {
                        index_js_1.OINOLog.warning("@oinots/db", "OINODbParser", "_createRowFromFormdata", "Form field not found and skipped!", { field_name: field_name });
                        block_ok = false;
                    }
                    else {
                        ...
                        while (block_ok && (l != '')) {
                            if (l.startsWith('Content-Type:') && (l.indexOf('multipart/mixed') >= 0)) {
                                index_js_1.OINOLog.warning("@oinots/db", "OINODbParser", "_createRowFromFormdata", "Mixed multipart files not supported and skipped!", { header_line: l });
                                block_ok = false;
                            }
                        ...
                        start += 2;
                        if (!block_ok) {
                            index_js_1.OINOLog.warning("@oinots/db", "OINODbParser", "_createRowFromFormdata", "Invalid block skipped", { field_name: field_name });
                        }
            ...
            if (has_data) {
                ...
                index_js_1.OINOLog.warning("@oinots/db", "OINODbParser", "_createRowFromFormdata", "Empty row skipped", {});
            }
        }
        catch (e) {
            index_js_1.OINOLog.exception("@oinots/db", "OINODbParser", "_createRowFromFormdata", "Exception parsing formdata", { message: e.message, stack: e.stack });
        }
        return result;
    }
        ...
        try {
            for (let i = 0; i < data_parts.length; i++) {
                ...
                if (param_parts.length == 2) {
                    ...
                    if (field_index < 0) {
                        index_js_1.OINOLog.info("@oinots/db", "OINODbParser", "_createRowFromUrlencoded", "Param field not found", { field: key });
                    }
                    else {
                        const field = datamodel.fields[field_index];
            ...
            if (has_data) {
                ...
                index_js_1.OINOLog.warning("@oinots/db", "OINODbParser", "_createRowFromUrlencoded", "Empty row skipped", {});
            }
        }
        catch (e) {
            index_js_1.OINOLog.exception("@oinots/db", "OINODbParser", "_createRowFromUrlencoded", "Exception parsing urlencoded data", { message: e.message, stack: e.stack });
        }
        return result;
    }
}
