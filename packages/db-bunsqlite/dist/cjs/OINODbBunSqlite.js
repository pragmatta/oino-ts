...
class OINODbBunSqlite extends db_1.OINODb {
    ...
    static _tableFieldTypeRegex = /[\"\[\s]?(\w+)[\"\]\s]\s?(INTEGER|REAL|DOUBLE|NUMERIC|DECIMAL|TEXT|BLOB|VARCHAR|DATETIME|DATE|BOOLEAN|JSON)(\s?\((\d+)\s?\,?\s?(\d*)?\))?/i;
    _db;
    /**
    ...
    async getSchemaFields(tableName) {
        ...
        const quoted_table = "\"" + tableName.replaceAll("\"", "\"\"") + "\"";
        const table_info = await this._query("PRAGMA table_info(" + quoted_table + ")");
        const pk_cols = [];
        while (!table_info.isEof()) {
            const ti_row = table_info.getRow();
        ...
        const fk_list = await this._query("PRAGMA foreign_key_list(" + quoted_table + ")");
        while (!fk_list.isEof()) {
            const fk_row = fk_list.getRow();
            const ref_table = fk_row[2]?.toString() || "";
            const from_col = fk_row[3]?.toString() || "";
            const to_col = fk_row[4]?.toString() || "";
            if (ref_table) {
                for (const f of fields) {
                    if (f.name == from_col) {
                        f.fieldParams.isForeignKey = true;
    ...
    printSqlCreateTable(tableName, fields) {
        ...
        const autoinc_pk = ((primary_keys.length == 1) && primary_keys[0].fieldParams.isAutoInc && (primary_keys[0] instanceof common_1.OINONumberDataField)) ? primary_keys[0] : null;
        const columns = [];
